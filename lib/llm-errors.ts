export function llmErrorMessage(error: unknown, timedOut = false): string {
  ...
  if (status === 401) return "Не удалось авторизоваться в Vercel AI Gateway. Проверьте AI_GATEWAY_API_KEY в Environment Variables и выполните Redeploy.";
  if (status === 402) return "Не удалось выполнить запрос: проверьте остаток кредитов Vercel AI Gateway и доступность модели на вашем тарифе.";
  if (status === 403 || status === 404) return "Не удалось получить доступ к модели. Проверьте AI_MODEL и доступность модели в вашем аккаунте Vercel AI Gateway.";
